...
namespace tardigradeMicromorphicLinearElasticity {
    ...
    class hydraMicromorphicLinearElasticity : public tardigradeHydra::hydraBaseMicromorphic {
       ...
       private:
        ...
        virtual void setResidualClasses() override {
            ...
            elasticity = tardigradeHydra::micromorphicLinearElasticity::residual(this, getConfigurationUnknownCount(),
                                                                                 *getParameters());
    ...
    void generate_input_variable_string(const std::vector<double> &time, const std::vector<double>(&fparams),
        ...
        for (auto t = time.begin(); t != time.end(); t++) {
            input_variables += " " + std::to_string(*t) + ",";
        }
        input_variables += "\nfparams:\n";
        for (auto f = fparams.begin(); f != fparams.end(); f++) {
            input_variables += " " + std::to_string(*f) + ",";
        }
        input_variables += "\ncurrent_grad_u:\n";
        for (unsigned int i = 0; i < 3; i++) {
            for (unsigned int j = 0; j < 3; j++) {
                input_variables += " " + std::to_string(current_grad_u[i][j]) + ",";
            }
            input_variables += "\n";
        }
        input_variables += "\ncurrent_phi:\n";
        for (unsigned int i = 0; i < 9; i++) {
            input_variables += " " + std::to_string(current_phi[i]) + ",";
        }
        input_variables += "\ncurrent_grad_phi:\n";
        for (unsigned int i = 0; i < 9; i++) {
            for (unsigned int j = 0; j < 3; j++) {
                input_variables += " " + std::to_string(current_grad_phi[i][j]) + ",";
            }
            input_variables += "\n";
        }
        input_variables += "\nprevious_grad_u:\n";
        for (unsigned int i = 0; i < 3; i++) {
            for (unsigned int j = 0; j < 3; j++) {
                input_variables += " " + std::to_string(previous_grad_u[i][j]) + ",";
            }
            input_variables += "\n";
        }
        input_variables += "\nprevious_phi:\n";
        for (unsigned int i = 0; i < 9; i++) {
            input_variables += " " + std::to_string(previous_phi[i]) + ",";
        }
        input_variables += "\nprevious_grad_phi:\n";
        for (unsigned int i = 0; i < 9; i++) {
            for (unsigned int j = 0; j < 3; j++) {
                input_variables += " " + std::to_string(previous_grad_phi[i][j]) + ",";
            }
            input_variables += "\n";
        }
        input_variables += "\nSDVS:\n";
        for (auto s = SDVS.begin(); s != SDVS.end(); s++) {
            input_variables += " " + std::to_string(*s) + ",";
        }
        input_variables += "\ncurrent_ADD_DOF:\n";
        for (auto a = current_ADD_DOF.begin(); a != current_ADD_DOF.end(); a++) {
            input_variables += " " + std::to_string(*a) + ",";
        }
        input_variables += "\ncurrent_ADD_grad_DOF:\n";
        for (auto a = current_ADD_grad_DOF.begin(); a != current_ADD_grad_DOF.end(); a++) {
            for (auto g = a->begin(); g != a->end(); g++) {
                input_variables += " " + std::to_string(*g) + ",";
            }
            input_variables += "\n";
        }
        input_variables += "\nprevious_ADD_DOF:\n";
    ...
    int evaluate_hydra_model(const std::vector<double> &time, const std::vector<double>(&fparams),
        ...
        try {
            ...
            PK2 = variableVector(hydra.getUnknownVector()->begin() + 0, hydra.getUnknownVector()->begin() + 9);

            SIGMA = variableVector(hydra.getUnknownVector()->begin() + 9, hydra.getUnknownVector()->begin() + 18);

            M = variableVector(hydra.getUnknownVector()->begin() + 18, hydra.getUnknownVector()->begin() + 45);
        ...
        try {
            ...
            for (unsigned int i = 0; i < 27; i++) {
                ...
                for (unsigned int j = 0; j < 27; j++) {
                    for (unsigned int k = 0; k < 27; k++) {
                        DMDgrad_phi[i][j] +=
                            (*dXdD)[numConfigurationUnknowns * (i + 18) + 18 + k] * dGradChidGradPhi[k][j];
                    }
                }
            }
